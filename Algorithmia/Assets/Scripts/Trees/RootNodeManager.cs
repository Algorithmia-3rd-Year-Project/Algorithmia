...
public class RootNodeManager : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private RootNodeLevelManager levelManager;
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (hit.collider.name == "Empty Root Node")
                {
                    currentObj = Instantiate(arrayblocksList.blockList["Empty Root Node"], new Vector3(mousePos.x, mousePos.y, 0f), Quaternion.identity);
