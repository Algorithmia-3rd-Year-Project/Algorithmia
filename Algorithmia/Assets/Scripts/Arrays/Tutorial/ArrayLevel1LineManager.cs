...
public class ArrayLevel1LineManager : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private ArrayLevel1BlockManager blockManager;
    ...
    [SerializeField] private InstructionLevel1 instructionManager;
            ...
            if (currentLine != null)
            {
                for (int i = 0; i < levelManager.lineEndPoints.Count; i++)
                {
                    ...
                    if (Mathf.Abs(mousePos.x - levelManager.lineEndPoints[i].position.x) <= 0.2f && Mathf.Abs(mousePos.y - levelManager.lineEndPoints[i].position.y) <= 0.2f && !parentLineEndStatus)
                    {
                        ...
                        instructionManager.lineCount += 1;
